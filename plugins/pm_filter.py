...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("𝗧𝗵𝗶𝘀 𝗕𝘂𝘁𝘁𝗼𝗻 𝗜𝘀 𝗡𝗼𝘁 𝗙𝗼𝗿 𝗬𝗼𝘂", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    ...
    if n_offset == 0:
        ...
        btn.append([InlineKeyboardButton(f"ᴘᴀɢᴇ {round(int(offset))+1} / {round(total/10)}", callback_data="pages"), InlineKeyboardButton("ɴᴇxT 🎗️", callback_data=f"next_{req}_{key}_{n_offset}")])
    else:
        btn.append(
            [
                InlineKeyboardButton("🎗️ Bᴀᴄᴋ", callback_data=f"next_{req}_{key}_{off_set}"),
                InlineKeyboardButton(f"ᴘᴀɢᴇ {round(int(offset))+1} / {round(total/10)}", callback_data="pages"),
...
async def advantage_spoll_choker(bot, query):
    ...
    if k==False:
        ...
        if files:
            ...
            k = await query.message.edit('𝚃𝚑𝚒𝚜 𝙼𝚘𝚟𝚒𝚎 𝙸𝚜 𝙽𝚘𝚝 𝙰𝚍𝚍𝚎𝚍 𝚃𝚘 𝙱𝚘𝚝 𝚈𝚎𝚝')
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons= [[
            ...
            InlineKeyboardButton('ᴏᴡɴᴇʀ', url='https://t.me/Robert_Pattinson999/19')
            ],[
            InlineKeyboardButton('« ʙᴀᴄᴋ', callback_data='start')
       ]]
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"ᴘᴀɢᴇs {round(int(total_results))}",callback_data="pages"), InlineKeyboardButton(text="ɴᴇxT 🎗️",callback_data=f"next_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
